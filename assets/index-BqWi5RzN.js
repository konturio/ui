const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./_virtual_cosmos-imports-BpBUqOEU.js","./_virtual_cosmos-imports-0UE6Ifno.css"])))=>i.map(i=>d[i]);
...
function isObject(data) {
    return isPlainObject(data) && !reactExports.isValidElement(data);
}
...
function stringifyUnserializableData(data) {
    ...
    return reactExports.isValidElement(data) ? '<React.Element />' : String(data);
}
...
function isMultiFixture(fixtureExport) {
    return (fixtureExport !== null &&
        ...
        !reactExports.isValidElement(fixtureExport) &&
...
function isReactElement(node) {
    return reactExports.isValidElement(node);
}
...
function findElementPaths(node, curPath = '') {
    if (Array.isArray(node)) {
        ...
        return flatten(Array.from(node).map((child, idx) => findElementPaths(child, `${curPath}[${idx}]`)));
    }
...
function isRefSupported(elementType) {
    ...
    return ($$typeof === Symbol.for('react.forward_ref') ||
...
function isNodeFixture(fixture) {
    ...
    return (fixture === undefined ||
        ...
        reactExports.isValidElement(fixture));
}
...
function DecoratedFixture({ fixture, fixtureOptions, userDecoratorModules, globalDecorators = [], }) {
...
function FixtureModule({ fixtureModule, decoratorModules, globalDecorators, fixtureId, initialFixtureState, renderKey, lazy, renderMessage, }) {
...
function RendererSync({ children, fixtures }) {
    ...
    React.useEffect(() => rendererConnect.onMessage(msg => {
        ...
        else if (msg.type === 'reloadRenderer' &&
            msg.payload.rendererId === rendererId) {
            reloadRenderer();
        }
...
function FixtureLoaderConnect({ moduleWrappers, selectedFixture, renderMessage, renderFixture, }) {
...
function ClientFixtureLoader({ moduleWrappers, globalDecorators, renderMessage = defaultRenderMessage, }) {
...
function useDomRendererConnect(webSocketUrl) {
    return React.useMemo(() => createDomRendererConnect(webSocketUrl), [webSocketUrl]);
}
function createDomRendererConnect(webSocketUrl) {
    if (typeof window !== 'undefined') {
        if (isInsideWindowIframe())
            return createPostMessageConnect();
        if (webSocketUrl)
            return createWebSocketsConnect(webSocketUrl);
    }
...
function getDomContainer(querySelector) {
    ...
    if (!existingContainer) {
        console.warn(`Query selector "${querySelector}" doesn't match any existing DOM element. ` +
...
async function mount() {
  const args = await __vitePreload(() => import('./_virtual_cosmos-imports-BpBUqOEU.js'),true?__vite__mapDeps([0,1]):void 0,import.meta.url);
  mountDomRenderer(args);
}

export { React as R, reactDomExports as a, React$1 as b, createValue as c, extendWithValue as e, getDefaultExportFromCjs as g, isEqual as i, reactExports as r, useFixtureState as u };
