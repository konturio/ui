...
const QuotientItem = ({ numeratorId, denominatorId, numeratorLabel, quality, onSelectQuotient }: QuotientItemProps) => {
  const onClick = (ev) => {
    ev.stopPropagation();
      ...
      {headingState.headingId === id && (
        <div className={styles.denominatorsContainer}>
          {quotients.map(({ id, label: numeratorLabel, quality }) =>
            id[0] !== selectedQuotient.id[0] || id[1] !== selectedQuotient.id[1] ? (
              <QuotientItem
...
const QuotientSelectorDisabled = ({ type }: { type: 'horizontal' | 'vertical' }) => (
  <div
    className={cn({
      [styles.denominators]: true,
      [styles.row]: type === 'horizontal',
...
export const QuotientSelector = memo(
  ({ id, quotients, selectedQuotient, onSelectQuotient, type }: QuotientSelectorProps) => {
